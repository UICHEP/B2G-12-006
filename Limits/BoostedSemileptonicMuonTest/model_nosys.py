...
def build_boosted_semileptonic_model(files, filter, signal, mcstat):
    ...
    #    model.add_lognormal_uncertainty('lumi', math.log(1.01), p)
    
    #model.add_lognormal_uncertainty('zj_rate', math.log(1.01), 'zlight')
    #model.add_lognormal_uncertainty('wj_rate', math.log(1.01), 'wlight')
    #model.add_lognormal_uncertainty('ttbar_rate', math.log(1.01), 'ttbar')
    #model.add_lognormal_uncertainty('st_rate', math.log(1.01), 'singletop')
    #model.add_lognormal_uncertainty('diboson_rate', math.log(1.01), 'diboson')    

    #for obs in ('mu_0btag_mttbar', 'mu_1btag_mttbar'):
    #    for proc in model.processes:
    #        model.add_lognormal_uncertainty('mu_eff', math.log(1.01), proc, obs)
...
def build_model(type, jet1, mcstat = True):
