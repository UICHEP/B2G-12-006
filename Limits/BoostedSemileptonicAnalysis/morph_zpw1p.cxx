...
{
    TFile f((channel + "_zpXw1p_interpolated.root").c_str(), "recreate");
    map<int, TH1*> zpmass_to_histo;
    zpmass_to_histo[500] = load(infilename, channel + "__zp500w1p");
    zpmass_to_histo[750] = load(infilename, channel + "__zp750w1p");
    zpmass_to_histo[1000] = load(infilename, channel + "__zp1000w1p");
    zpmass_to_histo[1250] = load(infilename, channel + "__zp1250w1p");
    zpmass_to_histo[1500] = load(infilename, channel + "__zp1500w1p");
    zpmass_to_histo[2000] = load(infilename, channel + "__zp2000w1p");
    zpmass_to_histo[3000] = load(infilename, channel + "__zp3000w1p");
    interpolate(&f, zpmass_to_histo, channel + "__zpXw1p", createInterpolatedHist, true);
    ...
    for(size_t isyst=0; isyst<syst.size(); ++isyst)
    {
        for(int ipm=0; ipm<2; ipm++)
        {
            ...
            zpmass_to_histo[500] = load(infilename, channel + "__zp500w1p__" + syst[isyst] + "__" + dir);
            if(!zpmass_to_histo[500]) continue;
            zpmass_to_histo[750] = load(infilename, channel + "__zp750w1p__" + syst[isyst] + "__" + dir);
            zpmass_to_histo[1000] = load(infilename, channel + "__zp1000w1p__" + syst[isyst] + "__" + dir);
            zpmass_to_histo[1250] = load(infilename, channel + "__zp1250w1p__" + syst[isyst] + "__" + dir);
            zpmass_to_histo[1500] = load(infilename, channel + "__zp1500w1p__" + syst[isyst] + "__" + dir);
            zpmass_to_histo[2000] = load(infilename, channel + "__zp2000w1p__" + syst[isyst] + "__" + dir);
            zpmass_to_histo[3000] = load(infilename, channel + "__zp3000w1p__" + syst[isyst] + "__" + dir);
            interpolate(&f, zpmass_to_histo, channel + "__zpXw1p__" + syst[isyst] + "__" + dir, createInterpolatedHist);
        }
    }
    f.cd();
    f.Write();
