...
{
    TFile f((channel + "_rsgX_interpolated.root").c_str(), "recreate");
    map<int, TH1*> zpmass_to_histo;
    zpmass_to_histo[1000] = load(infilename, channel + "__rsg1000");
    zpmass_to_histo[1500] = load(infilename, channel + "__rsg1500");
    zpmass_to_histo[2000] = load(infilename, channel + "__rsg2000");
    zpmass_to_histo[2500] = load(infilename, channel + "__rsg2500");
    zpmass_to_histo[3000] = load(infilename, channel + "__rsg3000");
    interpolate(&f, zpmass_to_histo, channel + "__rsgX", createInterpolatedHist, true);
    ...
    for(size_t isyst=0; isyst<syst.size(); ++isyst)
    {
        for(int ipm=0; ipm<2; ipm++)
        {
            ...
            zpmass_to_histo[1000] = load(infilename, channel + "__rsg1000__" + syst[isyst] + "__" + dir);
            if(!zpmass_to_histo[1000]) continue;
            zpmass_to_histo[1500] = load(infilename, channel + "__rsg1500__" + syst[isyst] + "__" + dir);
            zpmass_to_histo[2000] = load(infilename, channel + "__rsg2000__" + syst[isyst] + "__" + dir);
            zpmass_to_histo[2500] = load(infilename, channel + "__rsg2500__" + syst[isyst] + "__" + dir);
            zpmass_to_histo[3000] = load(infilename, channel + "__rsg3000__" + syst[isyst] + "__" + dir);
            interpolate(&f, zpmass_to_histo, channel + "__rsgX__" + syst[isyst] + "__" + dir, createInterpolatedHist);
        }
    }
    f.cd();
    f.Write();
