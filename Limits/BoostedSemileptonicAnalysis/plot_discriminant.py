...
class PropagateShapeSystematics:
    ...
    def get_process(self, channels, process):
        h_nominal = None
    ...
    def get_background(self, channels, processes, systematics, tolerance = 0.4):
        h_nominal = None
        h_plus = None
        h_minus = None
...
def plot(filename, xtitle, systematics):
    ...
    # loop over all the propagated channels
    for btag in ['0btag','1btag']:

        channels = ['mu_'+btag+'_mttbar', 'el_'+btag+'_mttbar']
        print "Systematics for btag %s" % btag 

        h_bkg = propagator.get_background(channels, backgrounds, systematics)
        h_sys = h_bkg.Clone()
        ...
        h_ttbar = propagator.get_background(channels, ['ttbar'], systematics)
        h_ttbar.SetLineColor(ROOT.kRed-3)
        h_ttbar.SetFillColor(ROOT.kRed-3)

        h_wjets = propagator.get_background(channels, ['wb', 'wc', 'wlight'], systematics)
        h_wjets.SetLineColor(ROOT.kGreen-3)
        h_wjets.SetFillColor(ROOT.kGreen-3)

        h_other = propagator.get_background(channels, ['zlight', 'singletop', 'diboson'], systematics)
        ...
        h_data = propagator.get_process(channels, 'DATA')
        h_data.SetLineColor(ROOT.kBlack)  
        h_data.SetMarkerStyle(20)

        h_zp1000 = propagator.get_process(channels, 'zp1000w1p')
        h_zp1000.SetLineWidth(3)
        h_zp1000.SetLineStyle(1)
        h_zp1000.SetLineColor(ROOT.kBlack)    

        h_zp2000 = propagator.get_process(channels, 'zp2000w1p')
        h_zp2000.SetLineWidth(3)
        h_zp2000.SetLineStyle(2)
        h_zp2000.SetLineColor(ROOT.kBlack)

        h_zp3000 = propagator.get_process(channels, 'zp3000w1p')
        ...
        labellumi2.SetHeader(channel_to_label[btag])
        labellumi2.Draw()
        ...
        canvas.Update()
        canvas.SaveAs('h_'+filename.split('.')[0]+'_'+btag+'.pdf')
